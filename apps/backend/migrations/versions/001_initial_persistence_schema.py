...
def upgrade() -> None:
    """Create baseline domain tables for users, exercise, reading, messaging, webhook, outbox."""
    op.create_table(
        "users",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("kakao_user_id", sa.String(length=255), nullable=False, unique=True),
        sa.Column("name", sa.String(length=120), nullable=False),
        sa.Column("timezone", sa.String(length=80), nullable=False, server_default="Asia/Seoul"),
    ...
    op.create_index("ix_exercise_plans_target_date_status", "exercise_plans", ["target_date", "status"])
    op.create_index("ix_exercise_plans_user_id_target_date", "exercise_plans", ["user_id", "target_date"])
    op.execute(
        sa.text(
            """
    ...
    op.create_table(
        "exercise_sessions",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("plan_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("exercise_plans.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        "exercise_set_states",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("session_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("exercise_sessions.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.UniqueConstraint("session_id", "set_no", name="uq_exercise_set_states_session_set_no"),
    )
    op.create_index("ix_exercise_set_states_session_id", "exercise_set_states", ["session_id"])
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="RESTRICT"), nullable=False),
        sa.Column("reading_plan_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("reading_plans.id", ondelete="SET NULL")),
    ...
    op.create_index("ix_reading_logs_user_created_at", "reading_logs", ["user_id", "created_at"])
    ...
    op.create_index("ix_notifications_user_status_schedule", "notifications", ["user_id", "status", "schedule_at"])
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.UniqueConstraint("provider", "idempotency_key", name="uq_webhook_events_provider_idempotency"),
    )
    op.create_index("ix_webhook_events_processed_created", "webhook_events", ["processed", "created_at"])
