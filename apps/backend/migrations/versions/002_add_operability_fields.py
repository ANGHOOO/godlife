...
def upgrade() -> None:
    ...
    op.add_column(
        "notifications", sa.Column("last_error_at", sa.DateTime(timezone=True))
    )
    ...
    op.add_column(
        "webhook_events",
        sa.Column(
            "schema_version", sa.String(length=16), nullable=False, server_default="v1"
        ),
    )
    ...
    op.create_index(
        "ix_webhook_events_signature_state", "webhook_events", ["signature_state"]
    )
    op.create_index(
        "ix_webhook_events_provider_schema",
        "webhook_events",
